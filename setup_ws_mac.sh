...
# --- 6b. Install and activate Zsh plugins (Autosuggestions & Syntax Highlighting) ---
...
# --- 12. Create default tmux config (USING YOURS) ---
TMUX_CONFIG_FILE="$HOME/.tmux.conf"
if [ -f "$TMUX_CONFIG_FILE" ]; then
  echo "✅ $TMUX_CONFIG_FILE already exists. Skipping."
else
  echo "Creating your preferred $TMUX_CONFIG_FILE..."
  cat << 'EOF' > "$TMUX_CONFIG_FILE"
# Added by Gemini for compatibility with the Zsh/P10k setup
set -g mouse on
...
if [[ "$response" == "y" || "$response" == "Y" ]]; then
    ...
    if [ -d "$HOME/Library/Application Support/Code/User" ]; then
        ...
        if jq \
          '.["terminal.integrated.fontFamily"] = "HackNdFont-Regular" | .["terminal.integrated.defaultProfile.osx"] = "zsh"' \
